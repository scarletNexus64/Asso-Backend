    ...
    if (pricingType === 'fixed') {
        html = `
            <div class="p-4 bg-dark-100 rounded-lg border border-dark-300">
                ...
                <input type="number" name="pricing_data[price]" id="pricing-data-price" required step="0.01" min="0"
                       placeholder="Ex: 1500"
                       class="w-full px-4 py-2 bg-dark-50 border border-dark-300 text-white rounded-lg focus:ring-2 focus:ring-primary-500">
            </div>
        `;
        ...
        for (const [category, description] of Object.entries(WEIGHT_CATEGORIES)) {
            html += `
                <div class="p-4 bg-dark-100 rounded-lg border border-dark-300">
                    ...
                    <input type="number" name="pricing_data[${category}]" id="pricing-data-${category.replace(/\s+/g, '-')}" required step="0.01" min="0"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    @if($deliverer->deliveryZones->count() > 0)
            ...
            const zoneHtml = `
                <div class="zone-item border border-dark-300 rounded-lg p-6 bg-dark-50" data-zone-id="${zoneId}">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex gap-2">
                            <input type="text" name="delivery_zones[${zoneId}][city]" id="zone-city-${zoneId}"
                                   value="${zoneData.city || ''}" required
                                   placeholder="Ex: Douala, Yaoundé, Bafoussam..."
                                   class="flex-1 px-4 py-2 bg-dark-100 border border-dark-300 text-white rounded-lg focus:ring-2 focus:ring-primary-500">
                            <button type="button" onclick="detectCity(${zoneId})"
                                    class="px-4 py-2 bg-primary-500 hover:bg-primary-600 text-white rounded-lg transition-colors flex items-center gap-2"
        ...
        @php
            $firstPricelist = $deliverer->deliveryZones->first()->pricelist ?? null;
            $pricingType = $firstPricelist->pricing_type ?? 'fixed';
            $pricingData = $firstPricelist->pricing_data ?? [];
        @endphp

        const currentPricingType = '{{ $pricingType }}';
        const currentPricingData = @json($pricingData);

        console.log('💰 Chargement des données de tarification:', currentPricingType, currentPricingData);

        // Trigger pricing fields update
        setTimeout(() => {
            updatePricingFields();

            // Load pricing data
            setTimeout(() => {
                if (currentPricingType === 'fixed' && currentPricingData.price) {
                    const priceInput = document.getElementById('pricing-data-price');
                    if (priceInput) {
                        priceInput.value = currentPricingData.price;
                    }
                } else if (currentPricingType === 'weight_category') {
                    for (const [category, price] of Object.entries(currentPricingData)) {
                        const inputId = `pricing-data-${category.replace(/\s+/g, '-')}`;
                    ...
                    if (rangesContainer) {
                        ...
                        volumetricRangeCounter = 0;
                    }

                    Object.entries(currentPricingData.ranges).forEach(([rangeKey, rangeData]) => {
                        const rangeId = volumetricRangeCounter++;
                        const container = document.getElementById('volumetric-ranges');
                        ...
                        const html = `
                            <div class="p-4 bg-dark-100 rounded-lg border border-dark-300" data-range-id="${rangeId}">
                                ...
                                <div class="grid grid-cols-3 gap-3">
                                    <div>
                                        <label class="block text-sm text-gray-400 mb-1">Poids min (kg)</label>
                                        <input type="number" name="pricing_data[ranges][${rangeId}][min]" value="${rangeData.min}" required step="0.01" min="0"
                                               class="w-full px-3 py-2 bg-dark-50 border border-dark-300 text-white rounded-lg text-sm">
                                    </div>
                                    <div>
                                        <label class="block text-sm text-gray-400 mb-1">Poids max (kg)</label>
                                        <input type="number" name="pricing_data[ranges][${rangeId}][max]" value="${rangeData.max}" required step="0.01" min="0"
                                               class="w-full px-3 py-2 bg-dark-50 border border-dark-300 text-white rounded-lg text-sm">
                                    </div>
                                    <div>
                                        <label class="block text-sm text-gray-400 mb-1">Prix (FCFA)</label>
                                        <input type="number" name="pricing_data[ranges][${rangeId}][price]" value="${rangeData.price}" required step="0.01" min="0"
                                               class="w-full px-3 py-2 bg-dark-50 border border-dark-300 text-white rounded-lg text-sm">
                                    </div>
                                </div>
                            </div>
