...
<div class="p-6">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="text-2xl font-bold text-white">Modifier le Livreur Partenaire</h1>
        </div>
        <p class="text-gray-400 ml-10">{{ $deliverer->name }} - {{ $deliverer->company_name }}</p>
    </div>
    ...
    <form action="{{ route('admin.deliverers.update', $deliverer) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="company_name" id="company_name"
                           value="{{ old('company_name', $deliverer->company_name) }}" required
                           class="w-full px-4 py-2 bg-dark-50 border border-dark-300 text-white rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('company_name') border-red-500 @enderror">
                    @error('company_name')
                ...
                <div>
                    ...
                    @if($deliverer->company_logo)
                        <div class="mb-3 flex items-center gap-3">
                            <img src="{{ Storage::url($deliverer->company_logo) }}" alt="{{ $deliverer->company_name }}"
                                 class="w-16 h-16 rounded-lg object-cover border border-dark-300">
                    ...
                    @error('company_logo')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-400">Laissez vide pour conserver le logo actuel</p>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="first_name" id="first_name"
                           value="{{ old('first_name', $deliverer->first_name) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="last_name" id="last_name"
                           value="{{ old('last_name', $deliverer->last_name) }}" required
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email"
                           value="{{ old('email', $deliverer->email) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="phone" id="phone"
                           value="{{ old('phone', $deliverer->phone) }}" placeholder="+229 XX XX XX XX"
                ...
                <div>
                    ...
                    @error('password')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-400">Laissez vide pour ne pas modifier</p>
                </div>
                ...
                <div>
                    ...
                    <select name="gender" id="gender"
                        ...
                        <option value="male" {{ old('gender', $deliverer->gender) == 'male' ? 'selected' : '' }}>Homme</option>
                        <option value="female" {{ old('gender', $deliverer->gender) == 'female' ? 'selected' : '' }}>Femme</option>
                        <option value="other" {{ old('gender', $deliverer->gender) == 'other' ? 'selected' : '' }}>Autre</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="birth_date" id="birth_date"
                           value="{{ old('birth_date', $deliverer->birth_date?->format('Y-m-d')) }}"
                           class="w-full px-4 py-2 bg-dark-50 border border-dark-300 text-white rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="country" id="country"
                        ...
                        @foreach([
                            ...
                            <option value="{{ $c }}" {{ old('country', $deliverer->country) == $c ? 'selected' : '' }}>{{ $c }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="bg-dark-100 rounded-xl shadow-lg p-6 mb-6 border border-dark-200">
            ...
            @include('admin.partials.google-map', [
                ...
                'latitude' => old('latitude', $deliverer->latitude),
                'longitude' => old('longitude', $deliverer->longitude),
                'address' => old('address', $deliverer->address),
                'zoom' => 13
            ])
        </div>
