...
<div class="p-6">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="text-2xl font-bold text-white">Modifier un Livreur Partenaire</h1>
        </div>
        <p class="text-gray-400 ml-10">Modifiez les informations du livreur, ses zones de livraison et tarifs</p>
    </div>

    <form id="delivererForm" action="{{ route('admin.deliverers.update', $deliverer) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="bg-dark-100 rounded-xl shadow-lg p-6 mb-6 border border-dark-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="company_name" value="{{ old('company_name', $deliverer->name) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="company_phone" value="{{ old('company_phone', $deliverer->phone) }}" required
                ...
                <div>
                    ...
                    <input type="email" name="company_email" value="{{ old('company_email', $deliverer->email) }}" required
                ...
                @if($deliverer->logo)
                    <div class="mt-2 mb-3">
                        <img src="{{ asset('storage/' . $deliverer->logo) }}" alt="Logo actuel" 
                             class="w-32 h-32 object-cover rounded-lg border-2 border-dark-300 shadow-lg">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="company_description" rows="3"
                              ...
                              placeholder="Décrivez l'entreprise de livraison...">{{ old('company_description', $deliverer->description) }}</textarea>
                </div>
            </div>
        ...
        <div class="bg-dark-100 rounded-xl shadow-lg p-6 mb-6 border border-dark-200">
            ...
            <div class="mb-6 p-6 bg-gradient-to-r from-primary-900/30 to-purple-900/30 border border-primary-500/50 rounded-lg">
                <div class="flex items-center justify-between">
                    <div>
                        @php
                            $latestSyncCode = $deliverer->syncCodes()->latest()->first();
                        @endphp
                        ...
                        <p class="text-xs text-gray-400 mt-2">
                            ...
                            @if($latestSyncCode)
                                @if($latestSyncCode->expires_at && $latestSyncCode->expires_at->isPast())
                                    <span class="text-red-400">Code expiré le {{ $latestSyncCode->expires_at->format('d/m/Y') }}</span>
                                @elseif($latestSyncCode->expires_at)
                                    Expire le {{ $latestSyncCode->expires_at->format('d/m/Y') }}
                                @else
                        ...
                        @if($latestSyncCode && $latestSyncCode->used_at)
                            <p class="text-xs text-green-400 mt-1">
                                <i class="fas fa-check-circle mr-1"></i>
                                Synchronisé le {{ $latestSyncCode->used_at->format('d/m/Y à H:i') }}
                            </p>
                        @endif
                    </div>
                    <div class="text-right">
                        @if(!$latestSyncCode || !$latestSyncCode->used_at)
                            ...
                            <p class="text-xs text-gray-400 mt-2">
                                @if($latestSyncCode)
                                    (Un nouveau code sera généré à la mise à jour)
                                @else
                                    (Le code final sera généré à la création)
                                @endif
                            </p>
                        @else
                            <div class="px-4 py-2 bg-green-900/30 border border-green-500/50 rounded-lg text-sm">
                                <i class="fas fa-check-circle text-green-400 mr-2"></i>
                                <span class="text-green-300">Code synchronisé</span>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="p-4 bg-gradient-to-r from-blue-900/30 to-primary-900/20 border border-blue-500/50 rounded-lg">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-blue-300 text-sm">
                            Un email professionnel sera automatiquement envoyé à l'adresse <strong>{{ old('company_email', $deliverer->email) }}</strong> avec le code de synchronisation.
        ...
        <div class="flex gap-4">
            <button type="submit" class="px-6 py-3 bg-gradient-to-r from-primary-500 to-primary-600 text-white rounded-lg hover:shadow-lg transition-all duration-200 shadow-lg hover:shadow-xl">
                <i class="fas fa-save mr-2"></i> Mettre à jour le livreur partenaire
            </button>
            <a href="{{ route('admin.deliverers.show', $deliverer) }}" class="px-6 py-3 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-all">
...
function initializeMapWithCoordinates(zoneId, lat, lng) {
    setTimeout(() => {
        const mapDiv = document.getElementById(`map-${zoneId}`);
        if (!mapDiv) {
            console.error(`❌ Map div not found for zone ${zoneId}`);
            return;
        }
        ...
        if (isNaN(latitude) || isNaN(longitude)) {
            console.error(`❌ Invalid coordinates for zone ${zoneId}:`, lat, lng);
            return;
        }

        console.log(`✅ Initializing map for zone ${zoneId} with coordinates:`, latitude, longitude);

        // Initialize map centered on the existing coordinates
        const map = L.map(`map-${zoneId}`).setView([latitude, longitude], 15);
        ...
        function placeMarker(lat, lng) {
            ...
            // Update hidden inputs
            document.getElementById(`center-lat-${zoneId}`).value = markerLat;
            document.getElementById(`center-lng-${zoneId}`).value = markerLng;
            ...
            marker.bindPopup(`
                <div style="text-align: center;">
                    <p style="font-weight: bold; margin-bottom: 5px;">📍 Position de la zone</p>
                    <p style="font-size: 12px; color: #666;">Lat: ${markerLat.toFixed(6)}</p>
                    <p style="font-size: 12px; color: #666;">Lng: ${markerLng.toFixed(6)}</p>
                    <p style="font-size: 10px; color: #999; margin-top: 5px;">Déplacez le marqueur si besoin</p>
                </div>
            `).openPopup();
        }

        // Place marker at existing coordinates
        console.log(`📍 Placing marker for zone ${zoneId} at:`, latitude, longitude);
                    ...
                    .then(results => {
                        ...
                        if (uniqueResults.length > 0) {
                                ...
                                <div class="p-2">
                                    ...
                                    <div class="space-y-1">
                                        ${uniqueResults.map((result, index) => {
                                            ...
                                            return `
                                                <div class="p-3 hover:bg-primary-900/30 cursor-pointer rounded-lg transition-colors ${index < uniqueResults.length - 1 ? 'border-b border-dark-300/30' : ''}"
                                                     onclick="selectSearchResult(${zoneId}, ${result.lat}, ${result.lon}, '${result.display_name.replace(/'/g, "\\'")}')">
                                                    <div class="flex items-start gap-3">
                                                        <div class="flex-shrink-0 w-8 h-8 bg-primary-500/20 rounded-full flex items-center justify-center mt-1">
                                                            <i class="fas fa-map-marker-alt text-primary-500 text-sm"></i>
                                                        </div>
                                                        <div class="flex-1 min-w-0">
                                                            <p class="text-sm font-semibold text-white">${mainName}</p>
                                                            <p class="text-xs text-gray-400 mt-0.5 line-clamp-1">${restName}</p>
                                                        </div>
                                                        <i class="fas fa-chevron-right text-gray-500 text-xs mt-2"></i>
                                                    </div>
                            ...
                            searchResults.innerHTML = `
                                <div class="p-6 text-center">
                                    <i class="fas fa-search text-gray-600 text-3xl mb-2"></i>
                                    <p class="text-gray-400 text-sm">Aucun résultat trouvé</p>
                                </div>
                            `;
                            searchResults.classList.remove('hidden');
                        }
...
document.addEventListener('DOMContentLoaded', function() {
    // Don't add a new zone if there are existing zones
    const hasExistingZones = {{ $deliverer->deliveryZones->count() > 0 ? 'true' : 'false' }};

    // Load existing zones if they exist
    @if($deliverer->deliveryZones->count() > 0)
        @php
            $existingZonesData = $deliverer->deliveryZones->map(function($zone) {
                return [
                    'id' => $zone->id,
                    'name' => $zone->name,
                    'center_latitude' => (float) $zone->center_latitude,
                    'center_longitude' => (float) $zone->center_longitude,
                    'pricing_type' => $zone->pricelist->pricing_type ?? 'fixed',
                    'pricing_data' => $zone->pricelist->pricing_data ?? [],
                ];
            });
        @endphp

        const existingZones = @json($existingZonesData);
        console.log('🗺️ Chargement des zones existantes:', existingZones);

        // Load existing zones
        existingZones.forEach((zoneData, index) => {
            const zoneId = zoneCounter++;
            const container = document.getElementById('zones-container');
            ...
            const zoneHtml = `
                <div class="zone-item border border-dark-300 rounded-lg p-6 bg-dark-50" data-zone-id="${zoneId}">
                    ...
                    <input type="hidden" name="delivery_zones[${zoneId}][id]" value="${zoneData.id}">

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-white mb-2">
                            Nom de la zone <span class="text-red-500">*</span>
                        </label>
                        <input type="text" name="delivery_zones[${zoneId}][name]" id="zone-name-${zoneId}"
                               value="${zoneData.name}" required
                               placeholder="Ex: Centre ville, Zone périphérique..."
                               onchange="updatePricelistZones()"
                               class="w-full px-4 py-2 bg-dark-100 border border-dark-300 text-white rounded-lg focus:ring-2 focus:ring-primary-500">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="relative mb-3">
                            <div class="relative">
                                <input type="text" id="search-input-${zoneId}" placeholder="🔍 Rechercher un lieu..."
                                       autocomplete="off"
                                       class="w-full px-4 py-3 pl-10 bg-dark-50 border-2 border-dark-300 text-white rounded-lg focus:ring-2 focus:ring-primary-500">
                                <i class="fas fa-search absolute left-3 top-1/2 transform -translate-y-1/2 text-primary-500"></i>
                                <div id="search-loader-${zoneId}" class="hidden absolute right-3 top-1/2 transform -translate-y-1/2">
                                    <i class="fas fa-spinner fa-spin text-primary-500"></i>
                                </div>
                        ...
                        <input type="hidden" name="delivery_zones[${zoneId}][center_latitude]" id="center-lat-${zoneId}"
                               value="${zoneData.center_latitude}" required>
                        <input type="hidden" name="delivery_zones[${zoneId}][center_longitude]" id="center-lng-${zoneId}"
                               value="${zoneData.center_longitude}" required>
                    </div>
                </div>
            `;

            container.insertAdjacentHTML('beforeend', zoneHtml);
            initializeMapWithCoordinates(zoneId, zoneData.center_latitude, zoneData.center_longitude);
            zones.push(zoneId);
        });

        // Update pricelists and set pricing data
        setTimeout(() => {
            console.log('💰 Mise à jour des tarifs pour les zones existantes');
            updatePricelistZones();

            existingZones.forEach((zoneData, index) => {
                const zoneId = index;
                console.log(`💵 Chargement des tarifs pour zone ${zoneId}:`, zoneData.pricing_type, zoneData.pricing_data);
                const pricingTypeSelect = document.getElementById(`pricing-type-${zoneId}`);
                if (pricingTypeSelect) {
                    pricingTypeSelect.value = zoneData.pricing_type;
                    updatePricingFields(zoneId);

                    setTimeout(() => {
                        if (zoneData.pricing_type === 'fixed' && zoneData.pricing_data && zoneData.pricing_data.price) {
                            const priceInput = document.querySelector(`[name="delivery_zones[${zoneId}][pricing_data][price]"]`);
                            if (priceInput) {
                                priceInput.value = zoneData.pricing_data.price;
                            }
                        } else if (zoneData.pricing_type === 'weight_category') {
                            for (const [category, price] of Object.entries(zoneData.pricing_data)) {
                                const input = document.querySelector(`[name="delivery_zones[${zoneId}][pricing_data][${category}]"]`);
                                if (input) {
                                    input.value = price;
                                }
                            }
                        } else if (zoneData.pricing_type === 'volumetric_weight' && zoneData.pricing_data.ranges) {
                            const rangesContainer = document.getElementById(`volumetric-ranges-${zoneId}`);
                            if (rangesContainer) {
                                rangesContainer.innerHTML = '';
                            }

                            Object.entries(zoneData.pricing_data.ranges).forEach(([rangeId, rangeData]) => {
                                if (!volumetricRangeCounters[zoneId]) {
                                    volumetricRangeCounters[zoneId] = 0;
                                }
                                const newRangeId = volumetricRangeCounters[zoneId]++;
                                const container = document.getElementById(`volumetric-ranges-${zoneId}`);

                                const html = `
                                    <div class="p-4 bg-dark-100 rounded-lg border border-dark-300" data-range-id="${newRangeId}">
                                        <div class="flex items-center justify-between mb-3">
                                            <span class="text-white font-medium">Plage #${newRangeId + 1}</span>
                                            <button type="button" onclick="removeVolumetricRange(${zoneId}, ${newRangeId})" class="text-red-400 hover:text-red-300 text-sm">
                                                <i class="fas fa-trash"></i>
                                            </button>
                                        </div>
                                        <div class="grid grid-cols-3 gap-3">
                                            <div>
                                                <label class="block text-sm text-gray-400 mb-1">Poids min (kg)</label>
                                                <input type="number" name="delivery_zones[${zoneId}][pricing_data][ranges][${newRangeId}][min]"
                                                       value="${rangeData.min}" required step="0.01" min="0"
                                                       class="w-full px-3 py-2 bg-dark-50 border border-dark-300 text-white rounded-lg text-sm">
                                            </div>
                                            <div>
                                                <label class="block text-sm text-gray-400 mb-1">Poids max (kg)</label>
                                                <input type="number" name="delivery_zones[${zoneId}][pricing_data][ranges][${newRangeId}][max]"
                                                       value="${rangeData.max}" required step="0.01" min="0"
                                                       class="w-full px-3 py-2 bg-dark-50 border border-dark-300 text-white rounded-lg text-sm">
                                            </div>
                                            <div>
                                                <label class="block text-sm text-gray-400 mb-1">Prix (FCFA)</label>
                                                <input type="number" name="delivery_zones[${zoneId}][pricing_data][ranges][${newRangeId}][price]"
                                                       value="${rangeData.price}" required step="0.01" min="0"
                                                       class="w-full px-3 py-2 bg-dark-50 border border-dark-300 text-white rounded-lg text-sm">
                                            </div>
                                        </div>
                                    </div>
            ...
            // Confirmation finale
            setTimeout(() => {
                console.log('✅ Chargement terminé! Zones chargées:', zones.length);
                console.log('📊 Détails des zones:', existingZones);
            }, 500);
        }, 200);
    @else
        // Add a new zone if none exist
        console.log('ℹ️ Aucune zone existante, ajout d\'une nouvelle zone');
        addZone();
    @endif
    ...
    form.addEventListener('submit', function(e) {
        ...
        const confirmMessage = `🎯 <strong>Mise à jour de l'entreprise de livraison</strong>
...
<div style="margin: 15px 0; padding: 15px; background: rgba(251, 191, 36, 0.1); border-left: 3px solid #f59e0b; border-radius: 8px;">
    ...
    • Modifier les zones de livraison et les tarifs
    • Les anciennes zones non incluses seront supprimées
</div>

<div style="margin: 15px 0; padding: 15px; background: rgba(99, 102, 241, 0.1); border-left: 3px solid #6366f1; border-radius: 8px;">
    ℹ️ <strong>Note :</strong> Le code de synchronisation existant restera valide. Un nouveau code ne sera pas généré sauf si vous en demandez un explicitement.
</div>`;
        ...
        logoInput.addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                reader.onload = function(event) {
                    // Créer ou mettre à jour l'image de preview
                    let preview = document.getElementById('logo-preview-img');
                    if (!preview) {
                        preview = document.createElement('img');
                        preview.id = 'logo-preview-img';
                        preview.className = 'mt-3 w-32 h-32 object-cover rounded-lg border-2 border-primary-500 shadow-lg';
